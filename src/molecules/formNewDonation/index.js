...
import { checkTitle, checkCity, checkAddres, checkNeighborhood, searchUf, searchCep, checkDesciption } from '../../utils/formValidation'
...
function FormDonation() {
  ...
  async function handleRegister(e) {
    ...
    if ((
      ...
      await api.post('donation', data, {
        ...
        .catch((error) => {
          console.log('Server error: ', error.response.status, error.response.data.error);
        });
    } else {
  ...
  async function useRegisteredAddress(e) {
    await api.get('donation/address', {
      headers: {
        authorization: token
      }
    })
      ...
      >
        <Fade in={modalCreate}>
          <div className={classes.paper}>
            <CheckCircle />
            <h2 id="transition-modal-title">Cadastro de doação realizado com sucesso!</h2>
          </div>
        </Fade>
      </Modal>
